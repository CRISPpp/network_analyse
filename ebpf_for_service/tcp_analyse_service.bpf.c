...
struct {
	...
	__uint(key_size, sizeof(u32));
	__uint(value_size, sizeof(u32));
} tcp_recvmsg_events SEC(".maps");

static int handle_tcp_v4_do_rcv(void *ctx, struct sock *sk)
{
	struct event event = {};
	u64 ts;
	...
	const char* description = "rcv ack";
	memcpy(event.tcp_description, description, sizeof(event.tcp_description));
	memcpy(event.func, "tcp_v4_do_rcv", sizeof(event.func));
...
static int handle_tcp_v4_rcv(void *ctx, struct sock *sk)
{
	struct event event = {};
	u64 ts;
	u32 tgid = bpf_get_current_pid_tgid() >> 32;
	...
	if (targ_min_us && event.delta_us < targ_min_us)
		goto cleanup;
	bpf_get_current_comm(&event.comm, sizeof(event.comm));
	memcpy(event.func, "tcp_v4_rcv", sizeof(event.func));
	const char* description = "server receive SYN packet";
	memcpy(event.tcp_description, description, sizeof(event.tcp_description));
	memcpy(event.tcp_state, get_tcp_state(state), sizeof(event.tcp_state));
	event.ts_us = ts / 1000;
	event.tgid = tgid;
...
static int handle_tcp_recvmsg(void *ctx, struct sock *sk)
{
	struct event event = {};
	u64 ts;
	u32 tgid = bpf_get_current_pid_tgid() >> 32;
	...
	if (targ_min_us && event.delta_us < targ_min_us)
		goto cleanup;
	bpf_get_current_comm(&event.comm, sizeof(event.comm));
	memcpy(event.func, "tcp_recvmsg", sizeof(event.func));
	const char* description = "server receive message";
	memcpy(event.tcp_description, description, sizeof(event.tcp_description));
	memcpy(event.tcp_state, get_tcp_state(state), sizeof(event.tcp_state));
	event.ts_us = ts / 1000;
	event.tgid = tgid;
	...
	bpf_perf_event_output(ctx, &tcp_recvmsg_events, BPF_F_CURRENT_CPU,
			&event, sizeof(event));
...
{
	return handle_tcp_v4_do_rcv(ctx, sk);
}

SEC("fentry/tcp_recvmsg")
int BPF_PROG(fentry_tcp_recvmsg, struct sock *sk) {
	return handle_tcp_recvmsg(ctx, sk);
}

SEC("fentry/tcp_v4_rcv")
int BPF_PROG(fentry_tcp_v4_rcv, struct sock *sk)
{
	return handle_tcp_v4_rcv(ctx, sk);
}
